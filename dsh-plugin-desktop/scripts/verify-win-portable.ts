...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `RunDeep-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'RunDeep.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing RunDeep.exe: ${portablePath}`)
  }
