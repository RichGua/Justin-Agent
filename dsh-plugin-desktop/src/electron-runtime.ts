...
/** Native adapter used by the RunDeep launcher and owned by its Cordis shell plugin. */
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  private async showRendererBootRecovery(report: Extract<RendererBootReport, { status: 'failed' }>): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      detail: `Failed plugins:\n${plugins}\n\n${error}\n\nOpen RunDeep Terminal to update or remove the failing third-party plugin, then restart RunDeep.`,
  ...
  private async confirmUpdateDownload(version: string): Promise<boolean> {
    const result = await dialog.showMessageBox({
      type: 'info',
      title: 'RunDeep Update Available',
      message: `RunDeep ${version} is available.`,
  ...
  private async showManualUpdateCheckResult(result: UpdateCheckResult | null): Promise<void> {
    if (result === null) {
      await dialog.showMessageBox({
        ...
        message: 'RunDeep could not check for updates.',
    ...
    if (result.status === 'up-to-date') {
      await dialog.showMessageBox({
        ...
        message: 'No newer version of RunDeep is available.',
    ...
    await dialog.showMessageBox({
      type: 'info',
      title: 'RunDeep Update Available',
      message: `RunDeep ${result.latestVersion} is available.`,
  ...
  private async downloadAndOpenUpdate(version: string, signal: AbortSignal): Promise<void> {
    ...
    if (platform === 'darwin') {
      ...
      await dialog.showMessageBox({
        ...
        message: `RunDeep ${version} is ready to install.`,
        detail: 'The disk image has opened. Replace RunDeep in Applications, then reopen it.',
        buttons: ['OK'],
        defaultId: 0,
        noLink: true,
      })
    ...
    const result = await dialog.showMessageBox({
      ...
      title: 'RunDeep Update Downloaded',
      message: `RunDeep ${version} is ready to install.`,
      detail: 'Restart RunDeep and run the installer now?',
  ...
  private async performUpdateArtifactCleanup(): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      message: zh
        ? `RunDeep ${artifact.version} 已安装。`
        : `RunDeep ${artifact.version} has been installed.`,
      detail: zh
        ? `是否删除下载的安装包以释放磁盘空间？\n\n${artifact.path}`
  ...
  private reportTerminalLaunchError(cause: unknown): void {
    ...
    try {
      dialog.showErrorBox('Unable to Open RunDeep Terminal', error.message)
    } catch (dialogCause) {
