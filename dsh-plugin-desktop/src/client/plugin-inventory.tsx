...
const zh = {
  ...
  harness: '归属', newHarness: '新增 Harness', harnessName: 'Harness 名称', enginePick: '引擎插件（可选）', createHarness: '添加', deleteHarness: '删除 Harness', harnessError: 'Harness 保存失败，请重试。',
  engine: '引擎', primary: '当前主引擎', common: '通用插件', commonHint: '通用插件不随 Harness 切换改变', lockedHint: '由 Harness 运行引擎设置控制',
} as const
...
const en: Record<LocaleKey, string> = {
  ...
  harness: 'Group', newHarness: 'New harness', harnessName: 'Harness name', enginePick: 'Engine plugin (optional)', createHarness: 'Add', deleteHarness: 'Delete harness', harnessError: 'The harness could not be saved. Try again.',
  engine: 'Engine', primary: 'Active primary harness', common: 'Common plugins', commonHint: 'Not affected by harness switches', lockedHint: 'Controlled by the harness runtime setting',
}
...
export function HarnessPluginInventory({ api, t }: Props) {
  ...
  return <section className="rundeepInventory">
    ...
    {plugins?.harnesses.map(harness => {
      ...
      const primary = harness.id === plugins.primaryHarness
      return <section className="rundeepInventoryGroup" key={harness.id}>
        <div className="rundeepInventoryGroupHeading">
          <button type="button" className="rundeepInventoryGroupHeader" aria-expanded={open} aria-label={`${t(open ? 'collapse' : 'expand')}: ${groupName(harness, t('common'))}`} onClick={() => setExpanded(current => ({ ...current, [harness.id]: !open }))}>
            <span>{groupName(harness, t('common'))}</span>
            {primary ? <b className="rundeepInventoryPrimary">{t('primary')}</b> : null}
          ...
          {!harness.builtIn ? <button type="button" className="rundeepInventoryDeleteHarness" aria-label={`${t('deleteHarness')}: ${harness.name}`} disabled={pendingHarness || pendingEntry !== undefined || primary} onClick={() => { void deleteHarness(harness.id) }}>{t('deleteHarness')}</button> : null}
        </div>
        {open ? <div className="rundeepInventoryGroupBody">
          {entries.length === 0 ? <p>{t('noPlugins')}</p> : <ul>
            {entries.map(entry => <li key={entry.entryId}>
              ...
              <label className="rundeepInventoryHarnessSelect"><span className="sr-only">{t('harness')}: {shortName(entry.moduleName)}</span><select value={entry.harnessId} disabled={pendingEntry !== undefined || pendingHarness || entry.engine} onChange={event => { void assign(entry.entryId, event.currentTarget.value) }}>
                {plugins.harnesses.map(option => <option key={option.id} value={option.id}>{groupName(option, t('common'))}</option>)}
              </select></label>
              <span className="rundeepInventoryState">{entry.locked ? t('lockedHint') : t(entry.enabled ? 'enabled' : 'disabled')}</span>
...
function installStyles() {
  ...
  style.textContent = `
.rundeepInventory{display:flex;flex-direction:column;gap:12px}.rundeepInventoryTools{display:grid;grid-template-columns:minmax(0,1fr) minmax(360px,1.2fr);gap:12px;align-items:end}
.rundeepInventorySearch,.rundeepInventoryHarnessForm label{display:flex;gap:10px;align-items:center}.rundeepInventorySearch span,.rundeepInventoryHarnessForm label span{font-size:13px;color:var(--dsw-alias-label-tertiary)}.rundeepInventorySearch input,.rundeepInventoryHarnessForm input,.rundeepInventoryHarnessForm select,.rundeepInventoryHarnessSelect select{min-width:0;flex:1;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-3);color:inherit;padding:8px 10px}.rundeepInventoryHarnessForm{display:grid;grid-template-columns:minmax(0,1fr) minmax(160px,.7fr) auto;gap:8px;align-items:end}.rundeepInventoryHarnessForm button,.rundeepInventoryDeleteHarness{border:1px solid var(--dsw-alias-border-l2);border-radius:7px;background:var(--dsw-alias-bg-layer-3);color:inherit;padding:7px 10px;cursor:pointer}
