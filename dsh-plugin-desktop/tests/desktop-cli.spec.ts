...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode before loading the fixed packaged CLI entry', async () => {
    ...
    const argv = ['/Applications/RunDeep', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/RunDeep',
  ...
  it('snapshots plugin installs launched from the built-in RunDeep Terminal', async () => {
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/RunDeep.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/RunDeep.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\RunDeep\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\RunDeep\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/RunDeep.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/RunDeep.app/Contents/Resources/app.asar.unpacked/package.json')
