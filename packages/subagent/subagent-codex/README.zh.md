...
生产环境的协议层有意只实现这一单次执行约定所需的 app-server 方法。运行时依赖与六个 optional-dependency alias 均锁定到 `@openai/codex@0.149.0` / `codex-cli 0.149.0`。普通安装会按当前操作系统与 CPU 选择一个载荷。平台包内容和大小会变化；安装固定版本而非宿主 `PATH` 二进制，才是兼容性边界。
...
- **兼容性由开发证据锁定**：Codex 0.149.0 是协议基线。升级时必须重新生成上游 schema 证据，并重新运行握手、答案选择、审批、取消、无密钥真实产品以及带密钥的 DeepSeek 随机数测试。
