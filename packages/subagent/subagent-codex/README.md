...
The production wire intentionally implements only the app-server methods required by this one-shot contract. The runtime dependency and all six optional-dependency aliases are pinned to `@openai/codex@0.149.0` / `codex-cli 0.149.0`. A normal install selects one payload for the current OS and CPU. Platform package contents and sizes vary; the installation pin, not a host `PATH` binary, is the compatibility boundary.
...
- **Compatibility is pinned by development evidence** — Codex 0.149.0 is the protocol baseline. An upgrade requires regenerating upstream schema evidence and rerunning handshake, answer-selection, approval, cancellation, keyless real-product, and credentialed DeepSeek nonce tests.
