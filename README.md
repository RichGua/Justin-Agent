...
- **原生扫码登录**：在终端通过 `justin wechat` 启动，扫描巨大且纯净的 ASCII 二维码即可完成设备登录。
...
- **自定义 App ID**：支持在配置向导中填入你真实的 `wechat_app_id`，保证通信协议的合法性。
...
justin setup
```
向导将引导你配置大模型 API 密钥、设定 Agent 名字，以及是否开启微信集成。
...
### 3. 启动微信助理
当你完成 `justin setup` 中的微信配置后，只需执行：
```bash
justin wechat
```
终端会打印出一个巨大的二维码。请使用微信扫码，你的手机端将提示确认登录“iLink Bot”设备。确认后，Justin 就会接管微信消息，开始自动为你回复。
...
- **Native QR Code Login**: Start by running `justin wechat` in the terminal, scan the large, clean ASCII QR code, and complete the device login.
...
- **Custom App ID**: Supports entering your real `wechat_app_id` in the setup wizard to ensure the legality of the communication protocol.
...
justin setup
```
The wizard will guide you to configure the LLM API key, set the Agent's name, and choose whether to enable WeChat integration.
...
After completing the WeChat configuration in `justin setup`, simply execute:
```bash
justin wechat
```
The terminal will print a huge QR code. Use WeChat to scan it, and your phone will prompt you to confirm logging into the "iLink Bot" device. Once confirmed, Justin will take over WeChat messages and start automatically replying for you.
