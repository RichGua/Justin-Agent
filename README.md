...
- **Hermes 风格的凭证接入**：`gateway setup` 现在同时支持 “扫码配对” 和 “手动填写 `account_id / token / base_url`” 两种方式，和 Hermes 的 Weixin setup 路径保持一致。
- **持久化会话**：扫码成功或手动录入后，账号凭证都会保存在本地，后续启动无需每次重新扫码。
...
- **Hermes 风格 DM / Group Policy**：配置项改成了更贴近 Hermes 的 `DM policy`、`Group policy`、`allowed users` 和 `allowed groups`，不再依赖自定义 `wechat_app_id`。
...
```
`justin gateway setup` 会完成 WeChat iLink 账号配对、自动回复前缀、`DM policy` 和 `Group policy` 设置。接入方式可以选扫码，也可以手动填写 `account_id / token / base_url`；完成后凭证会保存到本地。之后运行 `justin gateway start` 就能直接恢复网关；如果你更喜欢老入口，`justin wechat` 仍然可用，并会在缺少配对信息时自动引导扫码。
...
- **Hermes-style credentials flow**: `gateway setup` now supports both QR pairing and manual entry of `account_id / token / base_url`, which is much closer to the Hermes Weixin setup path.
- **Persisted sessions**: Once paired or entered manually, the account credentials are saved locally so subsequent starts can restore the gateway without forcing a fresh scan.
...
- **Hermes-style DM / Group policy**: Justin now uses `DM policy`, `Group policy`, `allowed users`, and `allowed groups` instead of a custom `wechat_app_id` field.
...
justin gateway start
```
`justin gateway setup` handles iLink pairing, reply-prefix settings, `DM policy`, and `Group policy`. You can either scan a QR code or enter `account_id / token / base_url` manually, and the resulting credentials are saved locally. After that, `justin gateway start` restores the saved credentials and starts listening immediately. The legacy shortcut `justin wechat` still works and will fall back to QR pairing when needed.
