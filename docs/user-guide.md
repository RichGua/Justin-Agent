...
从产品下载入口获取 macOS 或 Windows 安装包。安装后的 RunDeep 自带运行所需的 Electron、Node 和 DSH 依赖，普通用户不需要另行安装 Node.js 或 pnpm。
...
插件是给 DSH 添加能力的扩展包，例如模型、工具、界面和工作流。RunDeep 使用的就是官方 Harness 的插件体系，官方插件可以直接安装使用；多个插件遵循统一的约定，可以一起安装、一起工作。
...
在 RunDeep 托盘打开的终端中，裸 `dsh` 和不带 `--profile` 的 plugin 命令默认使用当前激活 profile：
...
显式 `--profile <name>` 始终优先。插件变更后需要重启 RunDeep，才能让新的 bundle 进入 Loader 组合。

## 打开终端

从托盘选择 **Open RunDeep Terminal**。macOS 会打开 Terminal，Windows 会优先使用 Windows Terminal，找不到时回退到 PowerShell 或命令提示符。
